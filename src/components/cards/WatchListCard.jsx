...
function WatchListCard({ anime }) {
  ...
  return (
    <article className="flex flex-col items-start w-full">
      <div
        className="relative w-full h-full max-h-[350px]"
        onMouseEnter={() => setShowPreviewCard(true)}
        onMouseLeave={() => setShowPreviewCard(false)}
      >
        <div
          onClick={() => removeFromWatchList(anime.animeId)}
          className="absolute top-0 right-0 z-30 cursor-pointer bg-black/50  rounded-full"
        >
          <CiCircleRemove className="text-4xl text-white" />
        </div>
        <Link to={`/details/${anime.animeId}`} className="relative block w-full h-full overflow-hidden group">
          <div
